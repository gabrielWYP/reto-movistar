...
document.querySelectorAll('.nav button').forEach(b=>b.onclick=()=>{state.view=b.dataset.view;document.querySelectorAll('.nav button').forEach(x=>x.classList.toggle('active',x===b));controls();load()});async function initialize(){try{let r=await fetch('/api/status'),j=await r.json();if(r.ok&&j.default_as_of_date)state.asOf=j.default_as_of_date}catch(e){}controls();load()}initialize();
</script></body></html>'''
