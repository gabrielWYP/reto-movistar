...
- [ ] 6.1 RED/GREEN: require one stable digest confirmation retry for `REQUIERE_VALIDACION`; changed evidence escalates to `MANUAL_REVIEW`.
- [ ] 6.2 RED/GREEN: add a checksummed one-shot operator checkpoint on the durable PVC and resume without duplicate steps after restart.
