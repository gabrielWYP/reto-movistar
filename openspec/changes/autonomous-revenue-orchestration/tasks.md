...
- [ ] 0.2 Select trusted analyst identity and immutable audit fields before E.

## Phase 1: Domain and Intake

- [ ] 1.1 RED: add `back/tests/unit/test_orchestration_domain.py` for immutable contracts and out-of-order rejection (Runs/Fixed sequence).
- [ ] 1.2 GREEN: create `domain/orchestration.py`, `persistence/repository.py`; REFACTOR invariants/docs.
...
- [ ] 3.1 RED: add `back/tests/integration/test_run_recovery.py` for all Runs recovery/idempotency/ownership scenarios.
- [ ] 3.2 GREEN: implement `application/orchestrator.py`: leases, digests, transitions, resume, background runner.
...
- [ ] 4.1 RED: add `back/tests/integration/test_run_api.py` for Runs/Final Review API scenarios.
- [ ] 4.2 GREEN: create `entrypoints/run_api.py`; wire `api.py`, identity, conflicts/review routes; REFACTOR OpenAPI docs.
