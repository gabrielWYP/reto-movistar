...
def load_dataset(path: Path) -> SoniaDataset:
    ...
    with zipfile.ZipFile(path) as outer:
        ...
        if nested:
            source: zipfile.ZipFile | None = zipfile.ZipFile(io.BytesIO(outer.read(nested)))
        else:
            source = outer
        try:
            contents = {
                logical_name: _decode_csv(source.read(next(name for name in source.namelist() if name.endswith(file_name))))
