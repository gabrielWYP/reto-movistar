...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        ...
        type=Path,
        required=True,
        help="Directorio de los 6 CSV oficiales o ZIP oficial",
    )
    result.add_argument("--as-of-date", default="2026-07-31", help="Corte ISO YYYY-MM-DD")
    result.add_argument(
        "--collections-response",
        type=Path,
        help="JSON AgentResponse público de Cobranzas (referencia futura, no recalcula su score)",
    )
    ...
    recovery.add_argument(
        "--dimension", choices=sorted(ALLOWED_DIMENSIONS), default="SEGMENTO_PAIS"
    )
...
def main() -> None:
    ...
    collections_payload = (
        json.loads(args.collections_response.read_text(encoding="utf-8"))
        if args.collections_response
        else None
    )
    ...
    if args.command == "data-quality":
        ...
        payload = service.recovery_intelligence(
            args.as_of_date, args.scope, args.dimension, args.top_n
        )
    elif args.command == "insights":
        payload = service.management_insights(args.as_of_date, args.dimension, args.top_n)
    else:
        payload = service.risk_concentration(
            args.dimension, args.metric, args.top_n, args.as_of_date
        )
