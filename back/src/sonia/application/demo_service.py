...
_TRANSITIONS: dict[tuple[DemoState, DemoAction], _Transition] = {
    ...
    (DemoState.RECONCILING, DemoAction.CONFIRM_PAYMENT): _Transition(
        next_state=DemoState.ANALYZING,
        progress=94,
        actor="Aprobación humana",
        title="Pago aplicado y caso conciliado",
        detail="La conciliación quedó confirmada y habilitó el análisis ejecutivo.",
        tone="green",
    ),
    (DemoState.ANALYZING, DemoAction.GENERATE_INSIGHTS): _Transition(
        next_state=DemoState.CLOSED,
        progress=100,
        actor="Agente BI",
        title="Indicadores ejecutivos generados",
        detail="El ciclo E2E terminó con conciliación, métricas y auditoría preservadas.",
        tone="green",
    ),
}
    ...
    return DemoScenarioResponse(
        ...
        agents=(
            ...
            AgentSnapshot(
                agent_id="bi",
                name="BI",
                specialty="Inteligencia operativa",
                summary="Consolida conciliación, cartera e ingresos en indicadores explicables.",
                status=AgentStatus.PENDING,
                confidence=100,
                processed_items=1,
                evidence_count=4,
