...
class BIService:
    ...
    def _metrics(rows: list[SnapshotInvoice], as_of: date) -> dict[str, Any]:
        ...
        overdue = sum(
            (row.balance for row in rows if row.invoice.due_at and row.invoice.due_at < as_of),
            Decimal(),
        )
        overdue_customers = {
        ...
        return [
            {
                ...
                "outstanding_balance": amounts[bucket],
            }
            for bucket in order
            if documents[bucket]
        ]
    ...
    def _quality(self, as_of: date) -> dict[str, Any]:
        ...
        if later:
            limitations.append(
                "Payments dated after as_of_date are excluded from balances and reported separately."
            )
        return {
            ...
            "as_of_exclusions": {
                ...
                "payments_after_as_of_amount_pen": sum(
                    (item.amount for item in later if item.currency == PEN), Decimal()
                ),
                "unmatched_payment_count": len(self.model.unmatched_payments),
                "unmatched_payment_amount_pen": sum(
                    (item.amount for item in self.model.unmatched_payments if item.currency == PEN),
                    Decimal(),
                ),
            },
    ...
    def _concentration(
        ...
        grouped: dict[str, dict[str, Any]] = defaultdict(
            lambda: {"amount": Decimal(), "customers": set(), "documents": 0}
        )
        customer_amounts: dict[str, dict[str, Any]] = defaultdict(
            lambda: {"amount": Decimal(), "accounts": set(), "documents": 0}
        )
        for row in rows:
            amount = (
                row.balance
    ...
    def data_quality_report(self, as_of_date: str | None = None) -> dict[str, Any]:
        ...
        evidence: list[dict[str, Any]] = [
            ...
            {
                "id": "dq_as_of_exclusions",
                "type": "as_of_exclusions",
        ...
        return AgentResponse(
            ...
            status={
                ...
                if quality["as_of_exclusions"]["unmatched_payment_count"]
                else "OK"
            },
            metrics={
                ...
                "payments_after_as_of_count": quality["as_of_exclusions"][
    ...
    def executive_snapshot(self, as_of_date: str | None = None) -> dict[str, Any]:
        ...
        evidence: list[dict[str, Any]] = [
            ...
            {
                "id": "dq_as_of_exclusions",
                "type": "payment_application_scope",
                "value": quality["as_of_exclusions"],
            },
        ]
        ...
        return AgentResponse(
            ...
            analysis_scope={
                "currency": PEN,
                "invoice_issued_on_or_before": as_of,
                "applications_on_or_before": as_of,
            },
            methodology={
                "deterministic": True,
                "balance": "invoice_total - eligible_payments - eligible_credit_notes, floored at zero",
            },
            upstream_inputs=self._upstream_inputs(),
        ).to_dict()
    ...
    def risk_concentration(
        ...
        rows = self.model.snapshot(as_of)
        total, concentration, top_customers, groups_available = self._concentration(
            rows, as_of, dimension, metric, top_n
        )
        quality = self._quality(as_of)
        evidence: list[dict[str, Any]] = [
            {
                "id": "concentration_by_dimension",
                "type": "grouped_metric",
                "dimension": dimension,
                "metric": metric,
                "value": concentration,
        ...
        return AgentResponse(
            ...
            metrics={
                "metric_total": total,
                "groups_returned": len(concentration),
            ...
            analysis_scope={
                ...
                "dimension": dimension,
                "metric": metric,
                "as_of_date_applied": True,
            },
            methodology={
                "deterministic": True,
                "pareto_sort": f"{metric} descending, label ascending on ties",
    ...
    def recovery_intelligence(
        ...
        metrics = self._metrics(rows, as_of)
        quality = self._quality(as_of)
        overdue_total, groups, top_customers, _ = self._concentration(
            rows, as_of, dimension, "overdue_balance", top_n
        )
        preventive_rows = [
            row
            for row in rows
            if row.balance > TOLERANCE and (not row.invoice.due_at or row.invoice.due_at >= as_of)
        ]
        preventive_balance = sum((row.balance for row in preventive_rows), Decimal())
        adjustment_rows = [
            row for row in rows if row.credited > Decimal() and row.balance > TOLERANCE
        ]
        ...
        evidence: list[dict[str, Any]] = [
            {
                "id": "recovery_top_customers",
                "type": "customer_pareto",
                "metric": "overdue_balance",
                "value": top_customers,
            },
            {
                "id": "recovery_concentration",
            ...
            {
                "id": "recovery_preventive_exposure",
                "type": "not_due_open_balance",
                "value": {
                    "balance": preventive_balance,
                    "customer_count": len({row.invoice.customer for row in preventive_rows}),
                    "document_count": len(preventive_rows),
                },
            },
            {
                "id": "recovery_document_adjustments",
                "type": "open_invoices_with_credit_notes",
                "value": {
                    "outstanding_balance": adjustment_balance,
                    "credit_notes_linked": adjustment_credits,
                    "customer_count": len({row.invoice.customer for row in adjustment_rows}),
                    "document_count": len(adjustment_rows),
                },
            },
            {
                "id": "dq_as_of_exclusions",
        ...
        return AgentResponse(
            ...
            metrics={
                ...
                "addressable_exposure": sum(
                    (item["overdue_balance"] for item in top_customers), Decimal()
                ),
                "top_n_customer_coverage": top_customers[-1]["cumulative_share"]
                if top_customers
                else Decimal(),
    ...
    def management_insights(
        ...
        evidence: list[dict[str, Any]] = [
            {
                "id": "management_executive_metrics",
                "type": "executive_metrics",
                "value": executive_metrics,
            },
            {
                "id": "management_dimension_concentration",
                "type": "dimension_concentration",
                "dimension": dimension,
                "value": group_rows,
            },
            {
                "id": "management_recovery_pareto",
                "type": "recovery_pareto",
                "value": recovery["evidence"][0]["value"],
            },
            {
                "id": "management_preventive_exposure",
                "type": "not_due_open_balance",
                "value": recovery["evidence"][2]["value"],
            },
            {
                "id": "management_document_adjustments",
                "type": "document_adjustment_context",
                "value": recovery["evidence"][3]["value"],
            },
            {
        ...
        if overdue > TOLERANCE:
            ...
            actions.append(
                {
                    "action": "govern_overdue_exposure",
                    "reason": "Review the documented overdue portfolio through focused recovery and exception-management workstreams.",
                    "evidence_refs": ["management_executive_metrics", "management_recovery_pareto"],
                }
            )
        if group_rows:
            leader = group_rows[0]
            ...
            actions.append(
                {
                    "action": "focus_dimension_governance",
                    "reason": "Start review with the leading concentration group and its largest customer exposures.",
                    "evidence_refs": [
                        "management_dimension_concentration",
                        "management_recovery_pareto",
                    ],
                }
            )
        ...
        if coverage > Decimal():
            ...
            actions.append(
                {
                    "action": "coordinate_top_exposure_workstream",
                    "reason": "Coordinate with Collections on the largest-exposure customers using its approved operational priorities where available.",
                    "evidence_refs": ["management_recovery_pareto"],
                }
            )
        if recovery_metrics["preventive_open_balance"]:
            ...
            actions.append(
                {
                    "action": "monitor_preventive_portfolio",
                    "reason": "Monitor the not-due exposure before its documented due dates.",
                    "evidence_refs": ["management_preventive_exposure"],
                }
            )
        if recovery_metrics["document_review_open_balance"]:
            ...
            actions.append(
                {
                    "action": "review_adjustment_context",
                    "reason": "Validate adjustment context before deciding the next business action for the affected documents.",
                    "evidence_refs": ["management_document_adjustments"],
                }
            )
        ...
        return AgentResponse(
            ...
            analysis_scope={
                ...
                "dimension": dimension,
                "as_of_date_applied": True,
                "sample_scope": "hackathon dataset",
            },
            methodology={
                "deterministic": True,
                "composition": [
