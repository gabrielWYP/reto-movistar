...
from .data import SoniaDataset, TABLE_FILES
...
TABLE_REQUIREMENTS: dict[str, set[str]] = {
    ...
    "payments": {"RAZON_SOCIAL", "COD_CUENTA", "FACTURA_AFECTADA", "FECHA_PAGO", "MONTO_PAGADO"},
    "invoices": {"RAZON_SOCIAL", "COD_CLIENTE", "COD_CUENTA", "NRO_DOC_FISCAL", "FECHA_EMISION", "FECHA_VTO", "CHARGE_TOTAL_AMOUNT"},
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "message": (
                ...
                else "Los archivos no se utilizaron; revisa las observaciones antes de volver a cargarlos."
            ),
        }
...
def _decode(content: bytes) -> str:
    for encoding in ("utf-8-sig", "utf-8", "latin1", "cp1252"):
        try:
            ...
            continue
    raise CsvValidationError("No se pudo leer el archivo CSV. Usa UTF-8, ANSI o Windows-1252.")
...
def _detect_table(filename: str, headers: list[str]) -> str:
    ...
    raise CsvValidationError(f"{filename}: no se puede identificar la tabla por sus columnas. Usa el nombre oficial del archivo.")
...
def _validate_rows(table: str, rows: list[dict[str, str]]) -> list[str]:
    ...
    for index, row in enumerate(rows, start=2):
        ...
        if len(errors) >= 10:
            return errors
    if table == "invoices":
        duplicates = [document for document, count in Counter(row["NRO_DOC_FISCAL"] for row in rows).items() if count > 1]
        if duplicates:
            errors.append("Existen facturas repetidas; no se cargaron para evitar sobrescribir información.")
    return errors


def load_uploaded_csvs(files: Iterable[tuple[str, bytes]], max_files: int, max_bytes: int) -> tuple[SoniaDataset | None, UploadReport]:
    """Validate a complete or partial CSV package without writing it to disk.

    Invoices are mandatory because every deterministic calculation starts there.
    Payments, credit notes and master tables are optional; missing optional tables
    are represented as empty data, never mixed with the previous dataset.
    """
    incoming = list(files)
    report = UploadReport()
    if not incoming:
    ...
    if sum(len(content) for _, content in incoming) > max_bytes:
        report.errors.append("Los archivos exceden el tamaño máximo permitido para una carga.")
        return None, report
    ...
    for filename, content in incoming:
        ...
        try:
            ...
            if table in tables:
                raise CsvValidationError(f"{filename}: se recibió más de un archivo para la tabla {table}.")
    ...
    if "invoices" not in tables:
        report.errors.append("Falta el archivo de facturas. Es obligatorio para calcular cartera, saldos y vencimientos.")
        report.accepted_tables.clear()
        return None, report
