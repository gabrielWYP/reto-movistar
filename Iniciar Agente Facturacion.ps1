...
if ($LASTEXITCODE -ne 0) {
    ...
    & $Python -m pip install -e (Join-Path $Root "back")
...
$env:SONIA_PORT = "8080"
$env:PYTHONPATH = Join-Path $Root "back\src"

$BackendArguments = @("-m", "uvicorn", "sonia.app:app", "--host", "127.0.0.1", "--port", "8080")
$Backend = Start-Process -FilePath $Python -ArgumentList $BackendArguments -WorkingDirectory $Root -WindowStyle Hidden -PassThru
try {
    $Ready = $false
    ...
    & $Python (Join-Path $Root "front\dev_server.py") --host 127.0.0.1 --port $Port --backend-host 127.0.0.1 --backend-port 8080
} finally {
