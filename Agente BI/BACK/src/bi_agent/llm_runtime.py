...
class OpenAIRuntime:
    def __init__(self, post: Callable[[dict[str, Any], str], dict[str, Any]] | None = None) -> None:
    ...
    def select_tool(self, question: str, as_of_date: str) -> dict[str, Any]:
        ...
        response = self._post(
            {
                "model": os.environ.get("SONIA_BI_MODEL", DEFAULT_MODEL),
                "store": False,
    ...
    def interpret(self, question: str, tool_result: dict[str, Any]) -> str:
                ...
                "input": [
                    {
                        ...
                        "content": [
                            {
                                "type": "input_text",
                                "text": f"Interpreta brevemente esta pregunta usando exclusivamente el resultado determinístico.\nPregunta: {question}\nResultado determinístico: {json.dumps(compact, ensure_ascii=False)}",
                            }
                        ],
                    }
                ],
        ...
        answer = cast(str, response.get("output_text", "")).strip()
        if not answer:
